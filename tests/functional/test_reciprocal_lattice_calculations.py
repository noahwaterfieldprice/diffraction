...
class TestCreatingReciprocalLatticeFromSequence:
    def test_can_create_reciprocal_lattice_from_sequence(self) -> None:
    ...
    def test_error_if_invalid_lattice_parameter_given(self) -> None:
        invalid_lattice_parameters = (
            *CALCITE_RECIPROCAL_LATTICE_PARAMETERS[:5],
            "abcdef",
        )

        with pytest.raises(ValueError) as exception_info:
            ReciprocalLattice(invalid_lattice_parameters)  # type: ignore[arg-type]
        assert (
            str(exception_info.value) == "Invalid lattice parameter gamma_star: abcdef"
        )


class TestCreatingReciprocalLatticeFromMapping:
    def test_can_create_reciprocal_lattice_from_dictionary(self) -> None:
        lattice_parameters = {
            "a_star": 1.4539,
            "b_star": 1.4539,
            "c_star": 0.3696,
...
class TestCreatingReciprocalLatticeFromDirectLattice:
    def test_can_create_reciprocal_lattice_from_direct_lattice(self) -> None:
        ...
        assert_almost_equal(
            reciprocal_lattice.lattice_parameters,
...
class TestReciprocalSpaceCalculations:
    def test_lattice_parameters_available_as_attribute(self) -> None:
        lattice = ReciprocalLattice(CALCITE_RECIPROCAL_LATTICE_PARAMETERS)

        assert lattice.lattice_parameters == CALCITE_RECIPROCAL_LATTICE_PARAMETERS

    def test_calculating_reciprocal_metric_tensor(self) -> None:
        lattice = ReciprocalLattice(CALCITE_RECIPROCAL_LATTICE_PARAMETERS)

        assert_array_almost_equal(lattice.metric, CALCITE_RECIPROCAL_METRIC, decimal=4)

    def test_calculating_unit_cell_volume(self) -> None:
        lattice = ReciprocalLattice(CALCITE_RECIPROCAL_LATTICE_PARAMETERS)
        a_star, _b_star, c_star, *_ = CALCITE_RECIPROCAL_LATTICE_PARAMETERS
        expected_volume = sqrt(3) / 2 * a_star * a_star * c_star

        assert_almost_equal(lattice.unit_cell_volume, expected_volume)

    def test_creating_reciprocal_lattice_vectors(self) -> None:
        lattice = ReciprocalLattice(CALCITE_RECIPROCAL_LATTICE_PARAMETERS)
        v1 = ReciprocalLatticeVector([1, 2, 3], lattice)
        v2 = lattice.vector([1, 2, 3])
        assert v1 == v2

    def test_calculating_length_of_reciprocal_lattice_vector(self) -> None:
        lattice = ReciprocalLattice(CALCITE_RECIPROCAL_LATTICE_PARAMETERS)
    ...
    def test_calculating_inner_product(self) -> None:
        lattice = ReciprocalLattice(CALCITE_RECIPROCAL_LATTICE_PARAMETERS)
    ...
    def test_calculating_angle_between_two_vectors(self) -> None:
        lattice = ReciprocalLattice(CALCITE_RECIPROCAL_LATTICE_PARAMETERS)
        v1 = ReciprocalLatticeVector([1, 0, 0], lattice)
        v2 = ReciprocalLatticeVector([0, 1, 0], lattice)
        v3 = ReciprocalLatticeVector([0, 0, 1], lattice)
    ...
    def test_calculating_inner_product_with_direct_lattice_vector(self) -> None:
        direct_lattice = DirectLattice(CALCITE_LATTICE_PARAMETERS)
        reciprocal_lattice = direct_lattice.reciprocal()
    ...
    def test_calculating_angle_with_direct_lattice_vector(self) -> None:
        direct_lattice = DirectLattice(CALCITE_LATTICE_PARAMETERS)
        reciprocal_lattice = direct_lattice.reciprocal()
        v1_direct = DirectLatticeVector([1, 0, 0], direct_lattice)
    ...
    def test_error_if_calculating_inner_product_with_different_lattices(self) -> None:
        ...
        assert (
            str(exception_info.value)
            == "ReciprocalLatticeVector and DirectLatticeVector"
